...
export default function SignUpEmailScreen({ navigation }: any) {
  ...
  const handleSendCode = async () => {
    ...
    setLoading(true);
    try {
      const response = await authService.sendSignupCode(trimmedEmail);

      if (response.status === 'success') {
        navigation.navigate('VerifyCode', { email: trimmedEmail, flow: 'signup' });
      } else {
          ...
          <View className="w-full max-w-[400px]">
            ...
            >
              <AppText className="text-[#ffffff] text-[14px]">
                ...
                <AppText className="text-[#ffffff] text-[14px] font-bold"> Login</AppText>
              </AppText>
            </TouchableOpacity>
          </View>
        </View>
