...
jest.mock('../../src/utils/featureFlags', () => ({
  isValidFeatureFlag: jest.fn((flag: string) => ['EXPERIMENTAL_FEATURE', 'DARK_MODE', 'ADVANCED_ANALYTICS'].includes(flag)),
}));
...
describe('UserController', () => {
  ...
  describe('getUserFeatureFlags', () => {
    ...
    it('should return empty array if no feature flags', async () => {
      (UserModel.findById as jest.Mock).mockResolvedValue({
        user_id: 1,
        username: 'testuser',
        feature_flags: null,
  ...
  describe('setUserFeatureFlags', () => {
    it('should set user feature flags', async () => {
    ...
    it('should remove duplicates from flags', async () => {
      (UserModel.setFlags as jest.Mock).mockResolvedValue({
        user_id: 1,
        feature_flags: ['DARK_MODE'],
      });
      ...
      const res = await request(app)
        .put('/api/users/1/feature-flags')
        .send({ flags: ['DARK_MODE', 'DARK_MODE', 'DARK_MODE'] });

      expect(res.status).toBe(200);
      expect(UserModel.setFlags).toHaveBeenCalledWith(1, ['DARK_MODE']);
    });

    it('should return 400 for invalid user id', async () => {
      const res = await request(app)
        .put('/api/users/invalid/feature-flags')
        .send({ flags: [] });
    ...
    it('should return 400 for invalid feature flag', async () => {
      const res = await request(app)
        .put('/api/users/1/feature-flags')
        .send({ flags: ['INVALID_FLAG'] });

      expect(res.status).toBe(400);
      expect(res.body.message).toContain('Invalid feature flag');
    });

    it('should return 404 if user not found', async () => {
      (UserModel.setFlags as jest.Mock).mockResolvedValue(null);
  ...
  describe('enableUserFeatureFlag', () => {
    it('should enable a feature flag', async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.status).toBe('success');
      expect(res.body.message).toBe('Feature flag enabled: DARK_MODE');
      expect(res.body.data.feature_flags).toEqual(['DARK_MODE']);
    });
    ...
    it('should return 404 if user not found', async () => {
      (UserModel.addFlag as jest.Mock).mockResolvedValue(null);
  ...
  describe('disableUserFeatureFlag', () => {
    it('should disable a feature flag', async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.status).toBe('success');
      expect(res.body.message).toBe('Feature flag disabled: DARK_MODE');
      expect(res.body.data.feature_flags).toEqual([]);
    });
    ...
    it('should return 404 if user not found', async () => {
      (UserModel.removeFlag as jest.Mock).mockResolvedValue(null);
