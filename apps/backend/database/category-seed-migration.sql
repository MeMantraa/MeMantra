...
-- Subcategories under "Mind & Emotional Health"
INSERT INTO "Category" ("name", "description", "category_type", "parent_id", "is_active")
SELECT sub.name, sub.description, 'essential', c.category_id, TRUE
FROM (VALUES
  ('Stress Relief / Calm', 'Mantras to reduce stress and find calm'),
  ('Mindfulness / Presence', 'Mantras for staying present and mindful'),
  ('Emotional Balance / Letting Go', 'Mantras for emotional equilibrium and releasing negativity'),
  ('Coping with Frustrations', 'Mantras for dealing with everyday frustrations')
) AS sub(name, description)
CROSS JOIN "Category" c
WHERE c.name = 'Mind & Emotional Health' AND c.category_type = 'essential' AND c.parent_id IS NULL
ON CONFLICT ("name", "category_type") DO NOTHING;

-- Subcategories under "Physical Health & Energy"
INSERT INTO "Category" ("name", "description", "category_type", "parent_id", "is_active")
SELECT sub.name, sub.description, 'essential', c.category_id, TRUE
FROM (VALUES
  ('Vitality / Energy', 'Mantras for boosting energy and vitality'),
  ('Exercise / Sports', 'Mantras for physical activity and sports performance'),
  ('Healthy Habits / Nutrition', 'Mantras for healthy eating and lifestyle habits'),
  ('Endurance / Strength', 'Mantras for physical endurance and inner strength')
) AS sub(name, description)
CROSS JOIN "Category" c
WHERE c.name = 'Physical Health & Energy' AND c.category_type = 'essential' AND c.parent_id IS NULL
ON CONFLICT ("name", "category_type") DO NOTHING;

-- Subcategories under "Work & Learning"
INSERT INTO "Category" ("name", "description", "category_type", "parent_id", "is_active")
SELECT sub.name, sub.description, 'essential', c.category_id, TRUE
FROM (VALUES
  ('Career / Job Challenges', 'Mantras for navigating career obstacles'),
  ('Productivity / Focus', 'Mantras to boost productivity and maintain focus'),
  ('Teaching / Learning', 'Mantras for educators and lifelong learners'),
  ('Skill Development / Growth', 'Mantras for developing new skills and growing professionally')
) AS sub(name, description)
CROSS JOIN "Category" c
WHERE c.name = 'Work & Learning' AND c.category_type = 'essential' AND c.parent_id IS NULL
ON CONFLICT ("name", "category_type") DO NOTHING;

-- Subcategories under "Relationships & Social Life"
INSERT INTO "Category" ("name", "description", "category_type", "parent_id", "is_active")
SELECT sub.name, sub.description, 'essential', c.category_id, TRUE
FROM (VALUES
  ('Romantic Relationships', 'Mantras for romantic partnerships and love'),
  ('Friends & Social Connections', 'Mantras for friendships and social bonding'),
  ('Communication & Empathy', 'Mantras for better communication and empathy'),
  ('Family / Extended Social Circle', 'Mantras for broader family and social networks')
) AS sub(name, description)
CROSS JOIN "Category" c
WHERE c.name = 'Relationships & Social Life' AND c.category_type = 'essential' AND c.parent_id IS NULL
ON CONFLICT ("name", "category_type") DO NOTHING;

-- Subcategories under "Parenting & Family"
INSERT INTO "Category" ("name", "description", "category_type", "parent_id", "is_active")
SELECT sub.name, sub.description, 'essential', c.category_id, TRUE
FROM (VALUES
  ('Parenting Challenges', 'Mantras for tackling parenting difficulties'),
  ('Family Harmony', 'Mantras for creating a peaceful family environment'),
  ('Patience & Guidance', 'Mantras for patience and guiding children'),
  ('Fun / Bonding Moments', 'Mantras for fun and bonding with family')
) AS sub(name, description)
CROSS JOIN "Category" c
WHERE c.name = 'Parenting & Family' AND c.category_type = 'essential' AND c.parent_id IS NULL
ON CONFLICT ("name", "category_type") DO NOTHING;

-- Subcategories under "Personal Growth & Goals"
INSERT INTO "Category" ("name", "description", "category_type", "parent_id", "is_active")
SELECT sub.name, sub.description, 'essential', c.category_id, TRUE
FROM (VALUES
  ('Confidence / Self-Belief', 'Mantras for building confidence and belief in yourself'),
  ('Resilience / Strength', 'Mantras for developing resilience and inner strength'),
  ('Life Goals / Ambition', 'Mantras for pursuing life goals and ambitions'),
  ('Self-Discovery / Learning', 'Mantras for self-exploration and personal learning')
) AS sub(name, description)
CROSS JOIN "Category" c
WHERE c.name = 'Personal Growth & Goals' AND c.category_type = 'essential' AND c.parent_id IS NULL
ON CONFLICT ("name", "category_type") DO NOTHING;

-- Subcategories under "Daily Life & Creativity"
INSERT INTO "Category" ("name", "description", "category_type", "parent_id", "is_active")
SELECT sub.name, sub.description, 'essential', c.category_id, TRUE
FROM (VALUES
  ('Routines / Daily Motivation', 'Mantras for daily routines and motivation'),
  ('Self-Care & Relaxation', 'Mantras for self-care and winding down'),
  ('Hobbies / Creative Expression', 'Mantras for hobbies and creative pursuits'),
  ('Celebrations / Small Wins', 'Mantras for celebrating achievements and small wins')
) AS sub(name, description)
CROSS JOIN "Category" c
WHERE c.name = 'Daily Life & Creativity' AND c.category_type = 'essential' AND c.parent_id IS NULL
ON CONFLICT ("name", "category_type") DO NOTHING;
